        ...
        <div class="col-md-12">
            @can('infrastructure_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.infrastructures.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $infrastructure->id }}">
                                        ...
                                        <td>
                                            @can('infrastructure_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.infrastructures.show', $infrastructure->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('infrastructure_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.infrastructures.edit', $infrastructure->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('infrastructure_delete')
                                                <form action="{{ route('frontend.infrastructures.destroy', $infrastructure->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('infrastructure_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.infrastructures.massDestroy') }}",
