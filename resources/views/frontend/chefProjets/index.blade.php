        ...
        <div class="col-md-12">
            @can('chef_projet_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.chef-projets.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $chefProjet->id }}">
                                        ...
                                        <td>
                                            @can('chef_projet_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.chef-projets.show', $chefProjet->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('chef_projet_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.chef-projets.edit', $chefProjet->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('chef_projet_delete')
                                                <form action="{{ route('frontend.chef-projets.destroy', $chefProjet->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('chef_projet_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.chef-projets.massDestroy') }}",
